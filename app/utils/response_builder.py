...
def build_clean_response(raw_data: dict):
    ...
    # -----------------------------------
    # 🔥 FIX 0: SUPPORT BOTH FORMATS
    # -----------------------------------
    payload = raw_data.get("payload")

    # If payload is string → convert to dict
    ...
    # ✅ If claim not present → use payload
    base_claim = raw_data.get("claim") or payload or {}
    ...
    case_data = raw_data.get("case") or {}
    payment = dict(raw_data.get("payment") or {})
    validation = dict(raw_data.get("validation") or {})

    # 🔥 FIX: ensure claim_id always present
    claim["claim_id"] = raw_data.get("claim_id") or claim.get("claim_id")

    denial_risk = claim.get("denial_risk", {})

    # -----------------------------------
    # 🔥 FIX 1: PRESERVE ORIGINAL STATUS
    # -----------------------------------
    final_status = raw_data.get("status", "SUCCESS")

    # -----------------------------------
    # ✅ FIX 2: Remove duplicate claim data
    # -----------------------------------
    pipeline.pop("claim", None)
    case_data.pop("claim", None)
    validation.pop("claim", None)
    ...
    if claim.get("patient", {}).get("name") in [None, "", "Unknown"]:
        errors.append({
            "field": "patient.name",
            "message": "Patient name missing"
        })
    ...
    # -----------------------------------
    # 🔥 FIX 4: FORCE HITL IF INVALID
    # -----------------------------------
    if not is_valid:
        final_status = "HITL_REQUIRED"

    # -----------------------------------
    # ✅ FIX 5: Correct pipeline steps
    # -----------------------------------
    steps = dict(pipeline.get("steps") or {})
    steps["rules_validated"] = is_valid

    # -----------------------------------
    # ✅ FIX 6: Financial cleanup
    # -----------------------------------
    reconciliation = payment.get("reconciliation", {})
    ...
    # -----------------------------------
    # ✅ FIX 8: Validation status
    # -----------------------------------
    validation_status = "passed" if is_valid else "failed"

    # -----------------------------------
    # 🔥 FIX 9: CLEAN CASE OUTPUT
    # -----------------------------------
    clean_case = None
    if case_data and case_data.get("case_id"):
        clean_case = {
    ...
    return {
        ...
        "financials": {
            "expected": reconciliation.get("expected"),
            "received": reconciliation.get("received"),
            "status": (reconciliation.get("status") or "").upper()
        },
